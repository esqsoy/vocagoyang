...
정답 공개와 따라쓰기 화면에서 IPA·듣기 버튼을 표시하고 자동 재생한다. 다음 문제로 넘어가면 아직 시작하지 않은 음성 예약을 취소한다. 이미 시작한 발화는 마칠 수 있으며 무음·종료·새 발화는 기존 음성을 취소한다. 원래의 1,500ms / 650ms 진행 시간은 유지한다.
...
첫 검사는 카드별 발음 누락·뜻 연결·표기 정리·원본과 HTML 일치를 확인한다. 두 번째 검사는 이번 변경 전의 데이터·판정·진도·시간 지문을 고정하고, 정답 노출 시점과 음성 동작을 실제 함수로 검사한다. 발음 데이터만 변경할 때는 JSON을 수정한 뒤 다시 조립한다.
