#!/usr/bin/env python3
import json,re
HTML='/home/claude/vocagoyang/vocagoyangfable.html'
src=open(HTML,encoding='utf-8').read()
data=[]
import os
if os.path.exists('out/lesson00.json'): data.append(json.load(open('out/lesson00.json')))
for i in range(1,5): data.append(json.load(open(f'out/lesson{i:02d}.json')))
for n in range(5,23):
    exs=json.load(open(f'out/set{n:02d}.json'))
    S=(n-5)*100+401; E=min((n-4)*100+400,2183)
    data.append({"lesson":n,"label":f"{n}세트","name":f"학습순서 {S}~{E}","exercises":exs})
