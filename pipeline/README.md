...
Fable(구 HoE, 0→4,639) + Mother Tongue(→약 8,200) = 95% 직독직해선. 그 위 **HoD(Heart of Darkness, →12,000, 콘래드)** → **GoE(Gift of English, →15,000, 나보코프 『재능』)**에서 종합 단어장 끝. 15,000→42,000(Brysbaert 2016 원어민 20세 수용 어휘)은 명작 한 권 = 단어장 한 권. 수특은 그 해 고3용이라 사다리 밖. 단위는 표제어. 인수인계 때 3층 이름이 빠져 데스크가 임시로 BoE라 부른 적 있음 — 폐기.
...
- 26.9.9 헌장 v2.1→v2.2(뜻 개수 상한 없음, 층 체계·표제어 단위 정리, 3층 이름은 v2.3에서 HoD(Heart of Darkness)로 복원). deduction 예외 추가, E-3 잔여 5건, 중복 검사 신설(trillion 중복 제거, 예문 재사용 4건 교체), 발음 강세 강조 복구(fmtIpa), meow 0~22 재정비 299줄 + 1~4세트 신설 68줄. 카드 5,695 · 표제어 4,639 · meow 595.
