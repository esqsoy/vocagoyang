...
현재 구성과 교육적 합의는 [WORK_PLAN.md](../WORK_PLAN.md)를 기준으로 한다. 현재 0세트 포함 51세트, 1,079연습, 6,886카드다. 원본은 520개 주제 묶음이며 각 묶음을 최대 8카드의 짧은 연습으로 나눈다. 카드 수·표제어 수·어족 수는 서로 다른 집계다.
...
DATA와 JSON의 exercises는 편집용 주제 묶음이다. 실제 연습은 HTML의 splitExercises(buildLessons())에서 최대 8카드, 묶음 안의 부분별 차이 1카드 이하로 구성한다. 원본 순서·카드 내용·과정 참조는 그대로 유지한다. 긴 연습은 `Exercise 1-1`, `1-2`처럼 표시하며 각 부분에 독립된 완료 키를 둔다. 예전 묶음의 클리어는 모든 부분에 적용하되 새 부분 하나를 끝냈다고 다른 부분까지 완료하지 않는다. 예전 이어하기 위치는 해당 묶음의 첫 미완료 부분으로 연결된다.
