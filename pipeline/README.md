# 보카고양 인수인계 (최종 갱신 26.9.17)

이 문서는 보카고양(Fable 5000) 개발·검수 역할을 새 세션으로 넘기기 위한 인수인계다. 원칙 하나: **이 문서 + GitHub 레포만 있으면 작업이 완전히 재개돼야 한다.** 파이프라인 원본(JSON)은 레포 `pipeline/out/`에 있고, 혹시 잃어도 HTML에서 복원하는 스크립트(`disassemble.py`)가 있다.

> **먼저 읽을 것 — 지금 어디까지 왔나 (26.9.17 현재)**
> 데이터는 1회전이 전부 끝났다. 기계 검사 넷(중의성 `collide.py` · 무결성 `deepcheck.py` · 예문 어휘 `exaudit.py` · 기본 `audit.py`)이 모두 통과 상태이고, 사람이 읽어야 보이는 층(4-3 B기준, 4-4 해설 필요성)도 전 세트 1회전을 마쳤다.
> **지금 국면은 영신의 직접 플레이 검수다.** 영신이 앱을 하면서 "단어+뜻번호"로 개별 카드를 보고하면 그걸 고친다(3-3 참고). 새 대형 작업을 먼저 벌이지 말 것.
> 미결 과제는 5절. 우선순위는 5번(접사·어근 확충)과 4번(SYN 표).
...
## 0-2. 층 체계 (헌장 제1조 v2.4, 26.9.10 영신 확정)
...
- 파일:
  ...
  - `vocagoyangfable.html` — **본체.** 단일 HTML(1,843,729자 = 2.31MB. `len()`은 글자를, 파일 크기는 바이트를 센다 — 한글이 3바이트라 둘이 크게 벌어진다. 26.9.16에 이걸 착각해 '파일이 갑자기 커졌다'고 오보한 적 있음), 안에 `const DATA = [...]` 블롭으로 48레슨 6,002카드(어휘 5,752 + 형태론 46·47세트 250)가 들어 있다. **DATA는 줄바꿈 없는 한 줄**이라 텍스트 편집이 아니라 JSON 왕복으로 고친다(3-2).
...
### 2-1. 정답 판정 — 손대기 전에 읽을 것

가장 아픈 버그가 두 번 다 여기서 났다. `submit()`의 정답 판정에는 **화면 상태·타이머 조건을 섞지 않는다.**

```js
let correct = !opt.forced && isCorrect(typed, w.term);
if(!correct && !opt.forced && typed.trim() && isSpellingVariant(typed, w.term)){correct=true; variantHit=true;}
if(!correct && !opt.forced && typed.trim() && isAliasHit(typed, w))          {correct=true; variantHit=true;}
```

26.9.15까지는 여기에 `!opt.timeout`이 붙어 있어서 **제한시간이 끝나는 순간 맞게 친 답이 오답으로 넘어갔다**(영신 보고: “맞춰도 따라쓰게 나와버리는데?”). `opt.timeout`은 “시간이 다 됐다”는 사실일 뿐 “틀렸다”가 아니다. 틀렸다는 뜻을 가진 것은 `opt.forced` 하나다.

더해서 `tick()`은 **치는 중이면 기다린다** — 마지막 타건(`s.lastKey`)이 1.2초 이내이고 입력창이 비지 않았으면 최대 5초까지 유예한다. `s.lastKey`를 심는 리스너는 `mirrorTyped`보다 **먼저** 달려야 하고 `nextCard`에서 0으로 지운다.

첫 시도 정답률(`s.firstCorrect`)도 이 판정을 그대로 쓰므로, 판정을 건드리면 기록도 함께 틀어진다.

## 3. 파이프라인 (레포의 `pipeline/` 폴더)

**26.9.17부터 파이프라인 전체가 레포 안에 있다.** 옛 작업 공간 `/home/claude/hoe-prod`도, zip 전달도 이제 없다. `git clone`만 하면 스크립트·원본 JSON·명단이 전부 따라온다.

스크립트는 **자기 위치를 기준으로 경로를 잡으므로 어느 폴더에서 실행해도 된다**(26.9.17 정리, 하드코딩 경로 전부 제거). `python3 /어디든/pipeline/audit.py …`로 부르면 된다.
...
### 3-1. 복원 · 첫 확인
```
git clone https://github.com/esqsoy/vocagoyang.git
cd vocagoyang
python3 pipeline/assemble.py      # → lessons 48 cards 6002 words 4713 bytes 1843729
git diff --stat                   # 아무 변화도 없어야 정상(왕복 무손실 확인)
```
`out/*.json`이 없거나 HTML과 어긋났을 때만 `python3 pipeline/disassemble.py`로 HTML에서 되뽑는다 — **이건 되돌리기이지 일상 절차가 아니다.**

### 3-2. 표준 수정 절차 (카드 고치기)

**철칙: 원본(JSON)과 HTML을 반드시 함께 고친다.** `pipeline/out/*.json`이 진실이고 HTML은 조립물이다. HTML만 고치면 다음 `assemble.py`에서 그 수정이 날아가고, JSON만 고치면 앱에 반영이 안 된다.

```
cd /home/user/vocagoyang
python3 pipeline/dump.py pipeline/out/set31.json        # 읽기
# edits.json: {"gross/1": {"tr": "…징그러워."}, "dot/1": {"ex": "Connect the {{BLANK}}s …"}}
python3 pipeline/apply.py pipeline/out/set31.json /경로/edits.json
python3 pipeline/audit.py pipeline/out/set31.json | tail -3    # 문제 0 확인
python3 pipeline/assemble.py                            # JSON → HTML 반영
node -e "const h=require('fs').readFileSync('vocagoyangfable.html','utf8');const s=h.match(/<script(?![^>]*src)[^>]*>([\s\S]*?)<\/script>/)[1];new Function(s);console.log('js ok')"
git add -A && git commit -m "…" && git push -u origin claude/브랜치명
```

**HTML의 DATA를 직접 만질 때는 JSON 왕복으로 한다.** 문자열 치환은 쓰지 않는다 — 같은 예문이 두 카드에 있으면 둘 다 바뀌고(실제로 겪음), DATA가 한 줄이라 `sed`도 못 쓴다.
```python
import json, re
h = open('vocagoyangfable.html', encoding='utf-8').read()
m = re.search(r'const DATA = (\[.*?\]);\n', h, re.S)
D = json.loads(m.group(1))
for L in D:
    for e in L['exercises']:
        for c in e['words']:          # 카드 배열의 이름은 words다(cards 아님)
            ...
blob = json.dumps(D, ensure_ascii=False, separators=(',', ':'))
open('vocagoyangfable.html','w',encoding='utf-8').write(h[:m.start(1)] + blob + h[m.end(1):])
```

**앱 코드(JS·CSS)를 고칠 때**는 HTML이 유일 원본이라 바로 고치면 된다. 고친 뒤 위의 `new Function(s)` 문법 검사와 scratchpad의 회귀 스크립트(`autotest.js` · `sweep.js` · `finalchk.js` · `integrity.js`)를 돌린다. 브라우저는 Chromium이 `/opt/pw-browsers/chromium`에 있고 `NODE_PATH=/opt/node22/lib/node_modules`로 playwright를 부른다(`playwright install` 금지).

함정 몇 가지:
- 셸 cwd가 호출 사이에 초기화된다 — 매번 `cd` 또는 절대경로.
- `grep`으로 예문을 찾을 땐 빈칸 어미 때문에 원문이 끊긴다(`{{BLANK}}d to be`).
- 파이썬 정규식 `\b`는 한글에 안 통한다(한글이 `\w`라 `"examination의"`가 `\bexamination\b`에 안 걸린다). 영단어 경계는 `(?<![a-z])X(?![a-z])`로 잡는다.
- f-string 안에 `%`가 들어간 CSS(`var(--x,50%)`)는 `%` 포매팅과 충돌한다 — 문자열 이어붙이기로.
...
### 4-2. 완성도 전수 검사 `pipeline/deepcheck.py` (26.9.17 신설)

4절(중의성)·`exaudit`(어휘) 밖의 기준들. 사람이 읽어야만 보이는 것 말고, **기계가 확실히 판정할 수 있는 것만** 넣었다.

| | 기준 | 26.9.17 결과 |
|---|---|---|
| A1 | 뜻 번호(si)가 1..sn 연속이고 sn이 실제 카드 수와 같은가 | 0건 |
| A2 | 같은 표제어·같은 품사의 IPA가 일관된가 | 0건(동철이음어 4개 면제 후) |
| A3 | pos가 빈칸의 실제 자리와 맞는가 | 0건 |
| A5 | 예문이 **세트를 넘어** 중복되지 않는가 | **25쌍 27장 적발 → 전부 교체** |
| A6 | 해설 72자·meow 48자·고양체·예문 10단어 | 0건 |

**A5가 이번 검사의 수확이다.** `audit.py`의 중복 검사는 세트 **안**에서만 돌아서, 서로 다른 세트의 두 카드가 같은 예문을 쓰는 것을 여태 못 잡았다. 같은 문장이면 빈칸의 답이 둘이라 학생이 어느 쪽을 쳐도 한쪽은 반드시 오답 처리된다 — 4절 기준으로 명백한 결함이다. 27장 전부 그 단어라야 자연스러운 문장으로 갈아 끼웠다.

**기계가 못 하는 것도 기록해 둔다.**
- **A4 tr↔뜻 대응은 설계에 실패했다.** 번역에 카드 뜻의 흔적이 있는지 문자열로 재려 했더니 '일곱'을 "7시 30분"으로, '모두'를 "다들"로, '지루해하는'을 "심심해"로 옮긴 멀쩡한 번역이 1,269건 걸렸다. 한국어의 형태 변화와 자연스러운 의역은 부분 문자열로 못 잡는다. 이 기준은 사람이 읽어야 한다.
- A2·A3의 첫 결과 24건은 **전부 오탐이었고, 데이터가 검사기보다 정확했다.** address(주소 ˈædrɛs / 연설 əˈdrɛs)·aged·bass·primer는 뜻마다 발음이 갈리는 것이 맞고, "go to school"·"listen to music"의 to는 부정사가 아니라 전치사다. 면제 규칙을 넣어 0건으로 만들었다.

사용: `python3 pipeline/deepcheck.py` (요약) · `--list` 대신 기준 이름을 인자로 (`A5`).

### 4-3. 사람이 읽어야 보이는 것 — B 기준 (26.9.17, 0세트 정독으로 도출)

기계 검사(4-1 중의성·4-2 무결성·exaudit 어휘)를 다 통과해도 남는 것들. **0세트 196장을 한 장씩 읽어서** 뽑았다.

| | 기준 | 0세트에서 걸린 것 |
|---|---|---|
| **B1** | 예문이 특정 종교·정치 입장을 전제하지 않는가 | `create①` "God created the world." → "This app was created by a high school student." |
| **B2** | 번역(tr)이 영어를 그대로 옮겼는가 — 회피하거나 외래어로 얼버무리지 않고 | `section①` "이 책의 이 **섹션**은" → "이 **부분**은"(ko가 '부분, 구역'인데 tr만 외래어) · `bitch①` "여자를 절대 **그 말로** 부르지 마"(빈칸이 그 단어인데 번역이 답을 가림) → "bitch라고" |
| **B3** | 같은 틀의 문장이 한 세트에 반복되지 않는가 | 수사 섹션의 "~ years old" 5회, "School starts at ~" 2회. 수사라 불가피한 면이 있어 두었다 |
| **B4** | 해설이 앱이 실제로 검증할 수 있는 것을 가르치는가 | `I①` c "항상 대문자 I" — 앱의 정답 판정은 대소문자를 무시해서(`normalize`가 소문자로 내린다) `i`를 쳐도 정답이다. **앱의 한계이지 카드의 결함은 아니다.** 고칠 거면 판정 쪽을 손대야 한다 |

읽으며 확인한 것: `CD①`·`DVD①`이 낡아 보이지만 c가 "요즘은 스트리밍이 대신한다"로 낡음을 명시하고 있어 의도된 설계다. 생활 구어 18장(욕설)도 c마다 "어른 앞·방송 금지"와 완곡어를 붙여 둔 의도된 구성이다.

**남은 질문 하나(영신 판단):** 욕설 카드는 학생이 빈칸에 그 단어를 **직접 타이핑**해야 한다. `fuck①` c는 "뜻만 알아 두고 쓰지 않는 것이 안전하다"고 하는데 앱은 치게 시킨다. 뜻만 알리려면 다른 방식(예: 빈칸을 다른 단어에 두고 욕은 문장 안에 노출)이 맞을 수도 있다.

### 4-4. 해설은 언제 필요한가 (26.9.17 영신과 합의)

**먼저 정리해 둘 것 — 정답일 때 해설을 감추자는 안은 폐기다.** 영신: "맞힌 건 그 단어 뜻에 대해서고, 해설은 오히려 '맞혀서 못 보게 되는' 게 단점이다. 이건 예전 토론의 결과물이다." `revealDelay = 1900 + 해설길이 × 75ms`도 버그가 아니라 **읽을 시간 예산**이다(75ms/자 ≈ 초당 13자). 손댈 것은 지연이 아니라 해설의 내용이다.

기준은 하나다: **이 한 줄이 없으면 학생이 무엇을 못 하게 되는가?** 답이 "아무것도"면 뺀다.

남는 것은 넷뿐이다.
1. **틀릴 뻔한 걸 막는다** — `Wednesday` 철자, `forty`의 u 없음, `visitor`(visiter 아님)
2. **뜻은 알아도 못 쓰는 걸 알려준다** — `discuss about(X)`, `prefer A to B`, `homework` 불가산
3. **모국어 간섭을 잡는다** — `note`/메모, `menu`/메뉴, `teacher`("Teacher!"로 부르지 않는다)
4. **기억을 걸어둘 고리를 준다** — `passport` = pass(통과)+port(항구)

**어원 분해의 칼날**(영신 확정): 조립한 결과가 표제어 뜻으로 **곧장 이어지면 빼고, 중간에 도약이 있으면 살린다.** 도약은 세 꼴로 온다 — ① 구성 요소가 학생이 아는 것과 다른 뜻(`classroom`의 class는 '수업'이 아니라 '학급') ② 구성 요소가 낯섦(`passport`의 port=항구) ③ 조립에서 뜻까지 비유가 있음. `dancer`=dance+r, `bedroom`=bed+room은 도약이 없어 뺀다.
→ 이 칼날은 **46·47세트(접사·어근)를 강화한다는 전제**와 짝이다. 체계적인 분해는 거기서 가르치고, 어휘 카드에는 위 세 경우만 남긴다.

**세트별 차등은 두지 않는다**(영신): Fable은 초보가 초·중등 어휘를 한 번에 해결하는 용도라 기준이 하나여야 한다.

**분해 칼날의 최종형**(26.9.17 영신 확정, `classroom`으로 다듬음):
① 구성 요소가 **각각 표제어이고 뜻도 같으면 뺀다** — `classroom`의 class는 3세트에 "① 수업, **반**"으로 이미 있다. '반'이 곧 '학급'이라 분해가 설 자리가 없다.
② 단, 조립에서 표제어 뜻까지 **비유의 도약**이 있으면 살린다 — `passport` = pass+port(항구)에서 '여권'까지는 건너뛰어야 한다.
③ 구성 요소의 뜻이 **표제어의 뜻과 다르면** 살린다 — `blog`의 log는 표제어 log(통나무)가 아니라 '기록'이다.

**세트마다 걸리는 병이 다르다.** 0세트는 분해(파생·합성)가, 1세트는 **해설 앞머리의 ko 되풀이**가 문제였다. 1세트에는 분해가 거의 없는 대신 `first①` "순서의 맨 앞이 first"처럼 뜻을 다시 말하는 첫 문장이 많았다. 세트를 열 때마다 "이 세트의 병은 무엇인가"부터 보는 게 맞다.

**전 세트 1회전 완료(26.9.17).** 0~45세트 5,751장을 세트별로 읽고 **254장**의 해설을 손봤다(통째 삭제 55장).

| | 전 | 후 |
|---|---|---|
| 해설 총량 | 210,880자 | 205,975자 (−2.3%) |
| 해설 없는 카드 | 549장 | 604장 |
| 공개 지연 평균 | 4.68초 | 4.62초 |

**세트별로 병이 달랐다** — 같은 체를 기계적으로 대면 놓친다.
- **0세트** 파생·합성 분해(dancer=dance+r, bedroom=bed+room)와 수사 해설의 숫자 되풀이("70.")
- **1~4세트** 해설 앞머리의 ko 되풀이(`first①` "순서의 맨 앞이 first", `use②` "명사로 쓰이면 '용도·쓰임'")
- **5세트** 문법어라 해설이 전부 핵심. **한 장도 손대지 않았다.**
- **6~20세트** 자명한 명사 파생 계보("X의 명사형")
- **21~45세트** 고급 어휘라 대부분 철자·발음 변화가 있어 살렸고, `-ing`/`-er` 꼴의 자명한 것만 걷어냈다

**되살린 것도 있다.** `marketplace①`의 해설에서 분해를 걷었더니 그 안에 있던 `market` 언급이 같이 사라져 중의성 후보가 하나 늘었다(`collide.py` 595 → 596). 분해가 우연히 대비 노릇을 하고 있었던 것이라, 대비를 제대로 쓴 해설로 바꿔 넣었다("the marketplace는 상업 세계 전체. 장터·개별 시장은 market"). **해설을 걷을 때는 그것이 다른 일을 겸하고 있지 않은지 확인해야 한다.**

**속도 이득은 여전히 작다(-1.3%).** 걷어낸 것이 대부분 앞머리의 짧은 군더더기여서다. 값어치는 속도가 아니라 학생의 눈이 닿는 첫 어절부터 정보가 있게 만드는 데 있다.

**첫 두 세트 실측(나머지 세트의 감각 기준)**
| | 0세트 196장 | 1세트 192장 |
|---|---|---|
| 손댄 카드 | 46장 | 40장 |
| 해설 통째 삭제 | 8장 | 9장 |
| 해설 총량 | 7,119 → 6,743자 (−5%) | 8,615 → 8,005자 (−7%) |
| 공개 지연 평균 | 4.63 → 4.48초 | 5.29 → 5.07초 |

**속도 이득은 작다는 것을 기록해 둔다.** 걷어낸 것이 대부분 앞머리의 짧은 군더더기("2.", "dance+r.")여서, 손댄 카드 수(45장)에 비해 글자 수 감소는 5%에 그쳤다. 나머지는 실제로 일하는 내용이었다. 그러니 이 작업의 값어치는 속도가 아니라 **학생의 눈이 닿는 첫 어절부터 정보가 있게 만드는 것**에 있다. 전 세트로 넓혀도 속도 그림은 비슷할 것이다.

## 5. 미결 과제

번호는 **고유 번호**다(순위가 아니다) — 한 번 붙인 번호는 그대로 두고 끝나면 취소선을 긋는다.
**지금 살아 있는 것은 4·5·6·7·8·11번.** 손이 빌 때 집을 순서는 영신의 뜻대로 **5번(접사·어근 확충) → 4번(SYN 표)**.
...
4. 명시 동의어표 `SYN` 신설 — **절반은 26.9.15에 코드 수정으로 해결**(4절 끝 참고, 안내 35→54쌍). 남은 **35쌍**은 한국어 뜻이 실제로 달라 표가 있어야 한다. 26.9.9 추가 후보: address/handle(다루다), address/speech(연설), get/arrive(도착하다), kind/sort/type.
5. 접사·어근(헌장 제7조) — ① 접사 46세트·② 어근 47세트(사촌 쌍 포함) **완료(26.9.10)**. **26.9.17 영신: 이쪽을 더 연구·확충하기로 했다.** 학생들이 아직 거기까지 가지 않아 시간 여유가 있고, 4-4의 '어원 분해 칼날'이 어휘 카드에서 분해를 걷어내는 만큼 체계적인 분해는 여기가 전담해야 한다. 남은 것: ③ 어족 묶어 돌리기 모드(앱, 단어 뷰와 함께), ④-2 불규칙 동사 패턴·④-3 구동사↔라틴 동사 대응(③의 갈래), ④-4 register(단어 뷰). 사양·후처리 스크립트는 `fable/morph/`.
...
10. ~~**기초 1~4세트 예문 어휘 경고**~~ — 완료(26.9.16). 영신 기준 확정: **"꼭 세트에 맞춰갈 필요는 없다. Fable 단어 수준이면 된다."** 그 기준으로 다시 재니 셋 중 둘은 애초에 문제가 아니었다 — `work② anymore`는 23세트, `real① fake`는 28세트 표제어라 통과. 남은 건 `love③ endless` 하나뿐이었고(표제어가 아예 아님), 겸사겸사 그 예문의 빈칸 중의성도 같이 잡았다(`A mother's ___ for her child is endless.`에는 care도 들어간다) → `I fell in ___ with her at first sight.`로 교체. fall in love with가 빈칸을 잠근다. 감사기 `pipeline/exaudit.py` 신설 — 허용 = 전 세트 표제어 + 화이트리스트 + 불규칙 + 수사. **0~45세트 5,752장 전수 위반 0장.**
11. **정답 효과의 꽃 — 보류 중, 차후 업데이트**(26.9.15 영신). 지금 게임에는 하트만 나간다(`FX.flowers=false`). 꽃 코드·여섯 연출·손잡이는 모두 살아 있고 한 줄로 돌아온다. 두 번 걸렸던 것: ① 크기 — 1.35배로 키웠다 ② 움직임 — 회전·시차·지속을 눌렀다(`calmFX`). 그래도 산만하다는 판단이라 끈 상태. 다시 볼 때 **크기 먼저, 그다음 움직임** 순서(영신). 손잡이판을 쓰면 화면 보면서 바로 돌릴 수 있다 — 상태줄 시계(00:00)를 1.5초 안에 네 번 탭, 또는 주소 끝에 `#fx`. 슬라이더를 움직이면 그 자리에서 다시 터지고, 맨 아랫줄이 그대로 코드에 넣을 값이다. **거리(`fly`)는 함부로 줄이지 말 것** — 실측으로 .68까지 내리면 꽃이 글자 위에 뭉쳐 앉아 가림이 평균 10%→27%(최대 121%)로 뛴다. 측정 스크립트는 세션 scratchpad의 `motion.js`·`occl.js`·`pkchk.js`.
  현재 손잡이(HTML 안 `var FX=`):
  ```js
  var FX={heartA:1, heartT:1, flowerA:1, flowerT:.72, petal:1.35, heartS:.94,
          fly:1, spin:.4, stagger:.5, flowers:false};
  ```
  `applyFX()`가 `--fx-ha/--fx-fa/--fx-pk`를 레이어에 심고, `calmFX(layer)`가 **첫 스타일 계산 전에** 파티클의 인라인 커스텀 속성을 훑어 곱한다(`CALM`의 네 무리: 거리 `fly` · 회전 `spin` · 지속 `dur` · 시차 `dly`). 크기 배율은 여섯 연출의 JS 수식이 아니라 **25개 파티클 클래스에 박은 `var(--fx-pk,1)` 하나**로 건다 — 수식을 여섯 군데 고치는 것보다 안전하다. `flowers:true` 한 글자면 꽃이 돌아온다.
## 6. 이력 요약 (git log 기준)

- 26.9.17 **해설 정리 1회전 전 세트**(`f24970f`) — 254장 손봄, 55장 통째 삭제. 기준과 세트별 병은 4-4. 해설 210,880 → 205,975자.
- 26.9.17 **완성도 전수 검사 `deepcheck.py` 신설**(4-2) — A1 si/sn 무결성, A2 IPA 일관성, A3 pos↔빈칸 자리, A5 예문 교차 중복, A6 형식. 다섯 기준 모두 0건. **A4(tr↔ko 대응)는 폐기** — 한국어 어간 추출이 거칠어 오탐 1,269건이 나왔다(“일곱”→“7시 30분”). 노이즈를 쏟는 대신 코드에 폐기 사유를 남겼다. 교차 중복 예문 27건은 이때 찾아 고쳤다(`audit.py`는 세트 안만 봤다).
- 26.9.17 **B기준 도출**(4-3) — 0세트 196장 정독. 기계가 다 통과시켜도 남는 층이 있다는 것을 확인.
- 26.9.16 **빈칸 중의성 기계 조사 `collide.py` 신설**(4-1) 및 0~45세트 후보 657건 1회전 — 12장 수정. 23세트 이후 적중률 0.
- 26.9.16 **예문 어휘 감사기 `exaudit.py` 신설**(5절 10번) — 0~45세트 5,752장 위반 0장.
- 26.9.15 **동의어 안내 35→54쌍**(`splitGloss` 3중 수정) — ① 괄호 안을 지우고 ② `;`·`또는`도 구분자로 보고 ③ 앞머리 뜻번호(①②③)를 떼도록. 오탐 0.02%.
- 26.9.15 **정답인데 따라쓰기로 넘어가던 버그**(영신 보고, 최우선 수정) — `submit()`의 정답 판정에 `!opt.timeout`이 붙어 있어 **제한시간이 끝나는 순간 맞게 친 답이 오답 처리**됐다. 그 조건을 떼고, 더해서 **치는 중이면 시간을 넘겨도 기다린다**(마지막 타건 1.2초 이내 + 입력창이 비지 않았으면 최대 5초 유예, `s.lastKey`). 첫 시도 정답률 집계도 같이 바로잡힘.
- 26.9.15 정답 효과 정리 — 꽃 1.35배·하트 0.94배로 비례를 맞추고, 움직임은 `calmFX`로 눌렀다(회전 −60%, 시차 −50%, 지속 −28%). **그래도 산만하다는 판단이라 꽃은 껐다**(`FX.flowers=false`) — 5절 11번. 쥐 타이머는 롤백(“시선을 뺏네”).
...
1. 이 문서와 헌장 아티팩트를 읽는다. 특히 맨 위의 “지금 어디까지 왔나”.
2. 레포를 clone하고 왕복이 무손실인지 본다:
   `python3 pipeline/assemble.py` → `lessons 48 cards 6002 words 4713 bytes 1843729` → `git diff --stat`이 비어야 한다.
3. 검사기 넷을 돌려 기준선을 잡는다(전부 지금 통과 상태다):
   ```
   python3 pipeline/audit.py pipeline/out/set45.json | tail -2   # 문제 0
   python3 pipeline/exaudit.py | head -3                          # 위반 0장
   python3 pipeline/deepcheck.py | tail -6                        # 다섯 기준 0건
   python3 pipeline/collide.py | tail -3                          # 후보 595(결함 수가 아니라 후보 수)
   ```
4. `git push -u origin claude/브랜치명`이 되는지 시험한다(이 세션 계보는 레포가 소스로 연결돼 직접 push된다).
5. 영신에게 한 줄: “복원 확인, 작업 가능. 뭐부터 할까?”
   — **먼저 큰 작업을 제안하지 말 것.** 지금 국면은 영신의 플레이 검수이고, 보고가 들어오면 그걸 고치는 게 일이다. 손이 빌 때 제안할 것은 5절 5번(접사·어근 확충) → 4번(SYN 표) 순.

## 8. 이 문서를 고치는 법

작업이 끝날 때마다 여기에 **결과가 아니라 판단 근거**를 남긴다. “무엇을 했다”보다 “왜 그렇게 했고, 무엇이 아니었나”가 다음 세션을 살린다. 실제로 이 문서에서 가장 값어치 있었던 줄들은 전부 그 꼴이다 — 꽃 거리(`fly`)를 줄였다가 가림이 늘었다는 실측, A4 기준을 폐기한 이유, 어두운 배경에서 반투명 채움이 때가 된다는 원칙, `revealDelay`가 버그가 아니라 읽을 시간 예산이라는 것.

영신의 판단으로 확정된 것은 **누가 언제 정했는지**를 같이 적는다(“26.9.17 영신 확정”). 나중에 그걸 뒤집으려 할 때 되돌아볼 자리가 된다.
