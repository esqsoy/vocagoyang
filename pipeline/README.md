...
현재 구성과 교육적 합의는 [WORK_PLAN.md](../WORK_PLAN.md)를 기준으로 한다. 현재 0세트 포함 51세트, 520연습, 6,886카드다. 카드 수·표제어 수·어족 수는 서로 다른 집계다.
...
채점에서 시간 종료 자체를 오답 조건으로 삼지 않는다. 강제 오답과 시간 종료는 구분한다. 한글 IME 조합 중에는 자동 제출하지 않는다. 공백을 입력 표시에 반영하되 정답 비교에서는 공백을 생략해도 인정한다. 기존 진도 키와 historical morphology progress/resume 연결을 보존한다.
