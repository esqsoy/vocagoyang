...
- 26.9.12 플레이 검수(영신): 0세트 player ① c의 'MP3 player'는 요즘 학생이 모름 → '게임 player, 동영상 player'로, ko '선수; 재생기'의 세미콜론 정리 → '선수, 플레이어'. 0세트 writer ① 예문 'She wants to be a ___.'에 author도 들어감 → 'She works as a ___ for a TV show.'(방송작가는 writer, 책의 저자는 author)로 교체, c에 'author' 대비 추가.
