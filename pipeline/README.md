...
현재 구성과 교육적 합의는 [WORK_PLAN.md](../WORK_PLAN.md)를 기준으로 한다. 현재 0세트 포함 51세트, 931연습, 6,886카드다. 원본은 520개 주제 묶음이며 각 묶음을 표제어가 갈라지지 않는 짧은 연습으로 나눈다. 카드 수·표제어 수·어족 수는 서로 다른 집계다.
...
DATA와 JSON의 exercises는 편집용 주제 묶음이다. 실제 연습은 HTML의 splitExercises(buildLessons())에서 구성한다. headwordRanges()는 같은 표제어의 첫 등장부터 마지막 등장 사이를 자르지 않는 경계만 허용한다. 그 경계 중에서 8카드에 가까운 분량과 여러 표제어의 혼합을 선호한다. 다의어를 한곳에 모으는 조건이 장수보다 우선하며 고정 상한은 없다. 현재 대부분 6~8카드, 최대 13카드다. 원본 순서·카드 내용·과정 참조는 유지한다.

긴 연습은 `Exercise 1-1`, `1-2`처럼 표시하며 각 부분에 독립된 완료 키를 둔다. 새 분할의 키는 heads-v2와 원본 카드 범위를 사용한다. 원래 묶음의 클리어는 모든 부분에 적용하고, 첫 분할판(short-v1)의 클리어는 새 연습을 덮는 모든 옛 부분이 완료됐을 때만 상속한다. 기록 조회는 저장된 데이터를 수정하지 않는다. saveLast는 layout을 함께 저장하며, 버전 없는 첫 분할판 이어하기는 같은 번호가 아니라 카드가 겹치는 위치로 연결한다. 별도 주제에서 의도한 재학습은 그대로 둔다.
